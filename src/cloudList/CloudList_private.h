...
void CloudList<T>::updateCloud(int index) {
	...
	qDebug() << "want to update: " << elements.at(index).dbId_;
 	Cloudbase::CBHelperSearchCondition* searchCond =
			new Cloudbase::CBHelperSearchCondition("cb_id", elements.at(index).dbId_.toStdString(),
...
void CloudList<T>::onInvoked(const bb::system::InvokeRequest& request) {
	...
	if (payload.isValid()) {
		...
		updateInternal(messageParts.at(0), messageParts.at(2), messageParts.at(3).toULongLong(),
				messageParts.at(1).compare("delete") == 0);
	}
}
