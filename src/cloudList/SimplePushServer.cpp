...
void SimplePushServer::pushMessageToUserList(const QString &id, const QString &operation, const QString &values, const qulonglong &stamp) {
	requestSubscribedUserList();
	QString papFormattedAddresses;
	populateAddresses(papFormattedAddresses);
	qDebug() << "addresses: " << papFormattedAddresses << " END";
	pushMessageToSpecifiedUsers(papFormattedAddresses, id, operation, values, stamp);
}
 ...
 *
 */
void SimplePushServer::pushMessageToSpecifiedUsers(const QString &papFormattedAddresses,
		const QString &id, const QString &operation, const QString &values, const qulonglong &stamp) {
	QString pushMessage;
	generatePushMessage(pushMessage, id, operation, values, stamp);
...
void SimplePushServer::generatePushMessage(QString& pushMessage,
		const QString &id, const QString &operation, const QString &values, const qulonglong &stamp) {
	pushMessage = QString("%0|%1|%2|%3").arg(id).arg(operation).arg(values).arg(stamp);
}
