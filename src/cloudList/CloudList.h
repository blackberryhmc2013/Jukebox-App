...
private:
	...
	{
	public:
		Element(T data, QString id = "", QDateTime stamp = QDateTime::currentDateTime());
...
void CloudList<T>::add(T data) {
	...
	// PUSH this add on the response handler
}
...
void CloudList<T>::remove(int index) {
	...
	updateCloud(index);
	pushServer->pushMessageToUserList(elements.at(index).dbId_, "delete" , "",
...
void CloudList<T>::update(int index) {
	elements[index].stamp_ = QDateTime::currentDateTime();
	updateCloud(index);
	pushServer->pushMessageToUserList(elements.at(index).dbId_, "update" , "",
			elements.at(index).stamp_.toMSecsSinceEpoch());
