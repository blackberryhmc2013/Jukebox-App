...
#include "WDListener.h"
#include <QThread>

WDListener::WDListener(QObject* parent)
: QObject(parent),
...
void WDListener::requestConnect(const QString& hardwareAddress)
{
...
void WDListener::readyServer()
{
	m_direct->requestStartSession();
	emit serverReady();
}

void WDListener::scanCompleted()
{
	qDebug() << "Scan completed!";
}

void WDListener::deviceFound(const QString& name, const QString& hardwareAddress, bb::device::WiFiDirectDeviceType::Type type)
{
	...
	if (bb::device::WiFiDirectDeviceType::Telephone == type) {
		foreach(QString id, phoneCons.keys()) {
			if (name.compare("BLACKBERRY-DCCF") == 0) {
				qDebug() << "found the server!";
...
void WDListener::deviceLost (const QString& hardwareAddress)
{
	qDebug() << "Device lost: " << hardwareAddress;
}

void WDListener::sessionStarted(const QString& sessionName)
{
	qDebug() << "Session started. Name: " << sessionName;
	m_session = sessionName;
}

void WDListener::deviceConnected (const QString& name, const QString& hardwareAddress, bb::device::WiFiDirectDeviceType::Type type,
...
void WDListener::deviceDisconnected(const QString& hardwareAddress)
{
	qDebug() << "Device Disconnected: " << hardwareAddress;
}

QString WDListener::resultToText(bb::device::WiFiDirectResult::Type result)
{
...
void WDListener::addTarget(QString phoneId)
{
	phoneCons[phoneId] = NULL;
}


bb::cascades::GroupDataModel* WDListener::model() const
{
    return m_model;
}

bool WDListener::stat() const
{
	return m_stat;
}

void WDListener::setStat(bool b)
{
	if (m_stat != b) {
		m_stat = b;
		emit statChanged();
